...
class Fx {
    ...
    public static HashMap<Integer,Boolean> seedCells(Tree tree, boolean includeAdj) {
        ...
        if(!includeAdj) {
            for(int adjSeed: Player.cells[tree.index].adjSeed[1].keySet())
                seeds.remove(adjSeed);
        }
...
class IncPartHeur {
    ...
    public static double growSunPts(Cell cell, int maxSize, boolean points, boolean dormant) {
        ...
        if(Player.debug)
            System.err.println("<nut:" + Player.nutrients + "> <rich:" + Player.richPts[cell.richness] +
                    "> <add: " + adder + ">");
...
class Player {
    ...
    public static final double getNutRatio = 17.0/15;
    /**
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            HashMap<Integer,Boolean> myFarSeedList = new HashMap<>();
            ...
            for(Tree tree: myTrees) {
                ...
                for(int seedIndex: Fx.seedCells(tree,false).keySet())
                    myFarSeedList.put(seedIndex,true);
            }
            ...
            while(!seedActions.isEmpty()) {
                ActionStatus seedMove = seedActions.poll();
                if(myTreeCount[0] == 0 && myFarSeedList.containsKey(seedMove.index) &&
                        seedMove.sunPts > move.sunPts) {
                    move = seedMove;
                    break;
                }
            }
